...
if docker ps | grep -q "$CONTAINER_NAME"; then
    ...
    echo ""
    echo "Please start Fulcrum first using:"
    echo "  ./run-fulcrum.sh                  # Without SSL"
    echo "  sudo ./run-fulcrum.sh              # With SSL (auto-detect)"
    echo "  ./run-fulcrum.sh --domain <domain> # With specific domain"
    echo ""
